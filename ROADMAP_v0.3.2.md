...
- [ ] The `X-Forwarded-For`-trusting pattern is copy-pasted across five files: `admin.py`, `contact.py`, `api.py`, `analytics.py`, and `login_throttle.py`. Each is independently spoof-bypassable on non-proxied deployments. v0.3.1 Phase 22.6 landed the fix on `admin.py` only as an interim.
...
- [ ] Replace every call site; delete the inlined duplicates (including the interim copy in `admin.py`). Preserve the behaviour for the login throttle hash.
...
- [ ] `CHANGELOG.md` note: operators with a weak key will now fail to start after upgrade; `manage.py rotate-secret-key` is the escape hatch. Call this out in the v0.3.2 release notes as a **Breaking change** entry.
