...
def create_app(config_path=None):  # noqa: C901 — app factory is inherently sequential setup; splitting into sub-functions would scatter related init logic without reducing real complexity
    ...
    def set_security_headers(response):
        ...
        # Phase 24.4 (#14) — strip the Server header so the exact
        # Gunicorn / Werkzeug version is not disclosed to every
        # response. Also drop X-Powered-By if any middleware set it.
        # Gunicorn's own `server_tokens` flag cannot be set from code,
        # but the header arrives as a `Server:` response entry that
        # our app is allowed to overwrite. A fully-empty value is the
        # cleanest — browsers and proxies don't error on its absence.
