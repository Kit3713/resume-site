...
- [x] **Brute-force alert:** `ResumeBruteForce` (warning/security) fires when `rate(resume_site_login_attempts_total{outcome="invalid"}[5m]) > 0.1` (~6/min) over 5 minutes. Rather than adding an `endpoint` label to `errors_total` (which would have meant changing the request-hook call site in `app/__init__.py`), the sharper approach was a dedicated counter: `resume_site_login_attempts_total{outcome}` where `outcome` is `success`, `invalid`, or `locked`. Increments emit from `app/services/login_throttle.py` — `record_failed_login` / `record_successful_login` / `check_lockout` (the last only when it actually refuses an attempt). This distinguishes credential-stuffing from the broader `AuthError` class that `ResumeAuthErrorSpike` watches, and keeps the 18.9 cardinality budget intact. Runbook section in `alerting-rules.md` explains how to triage the two alerts together.
