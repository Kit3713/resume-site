...
from tests.conftest import _write_test_config, _init_test_db
...
def test_csrf_admin_login_post_without_token_rejected(csrf_client):
    ...
    response = csrf_client.post('/admin/login', data={
        'username': 'admin',
...
def test_csrf_review_post_without_token_rejected(csrf_client):
    ...
    response = csrf_client.post('/review/fake-token-xyz', data={
...
def test_content_save_strips_xss(auth_client, populated_db):
    ...
    auth_client.post('/admin/content/edit/about', data={
        'title': 'About',
